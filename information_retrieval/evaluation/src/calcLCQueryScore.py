...
queryFile   = open(sys.argv[1],'r',encoding='mac_roman')
...
#        lineno += 1 
        ...
        if len(keywords)==1 and len(keywordStr) > 3 and keywordStr in queryLower:
              exactQueryWords=query.lower().split(' ')
...
#                 f.write(str(query) + " | " + str(keywordStr) + " | " + str(keywordScore) + " | " + str(score) + "\n")
        ...
        elif  len(keywords)==2 and len(keywords[1]) > 1 and keywordStr in queryLower:
#              str_concat = ' ' + keywords[0] + ' ' 
#              print( '*' + str_concat + '*' + '\n')
              if len(keywords[0]) < 3 and (' ' + keywords[0] + ' ') in queryLower:
                  count += 1
                  score += int(keywordScore)
#                 f.write(str(query) + " | " + str(keywordStr) + " | " + str(keywordScore) + " | " + str(score) + "\n")
                  print(str(query) + " | " + str(keywordStr) + " | " + str(keywordScore) + "\n")
...
#                 f.write(str(query) + " | " + str(keywordStr) + " | " + str(keywordScore) + " | " + str(score) + "\n")
                  print(str(query) + " | " + str(keywordStr) + " | " + str(keywordScore) + "\n")
        elif  weight_type == "ALL" and len(keywords) > 2 and keywordStr in queryLower:
              count += 1
              score += int(keywordScore)
#             f.write(str(query) + " | " + str(keywordStr) + " | " + str(keywordScore) + " | " + str(score) + "\n")
              print(str(query) + " | " + str(keywordStr) + " | " + str(keywordScore) + "\n")
